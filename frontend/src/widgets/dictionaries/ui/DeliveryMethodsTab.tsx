...
import { DeliveryMethodForm } from '@/features/dictionaries/ui/DeliveryMethodForm'
import { getDeliveryMethods, createDeliveryMethod, updateDeliveryMethod, activateDeliveryMethod, deactivateDeliveryMethod } from '@/entities/delivery-method/api/deliveryMethodApi'
...
export function DeliveryMethodsTab() {
  const [items, setItems] = useState<DeliveryMethod[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState<'all' | 'active' | 'inactive'>('active')
  const [sheetOpen, setSheetOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<DeliveryMethod | undefined>()
  const [saving, setSaving] = useState(false)
  const formRef = useRef<HTMLFormElement>(null)

  const load = useCallback(async (s: string, sf: typeof statusFilter) => {
    setLoading(true)
    try { setItems(await getDeliveryMethods({ isActive: sf === 'all' ? undefined : sf === 'active', search: s || undefined })) }
  ...
  const handleSave = async (data: CreateDeliveryMethodDto) => {
    setSaving(true)
    try {
      if (editingItem) { await updateDeliveryMethod(editingItem.id, data); toast.success('Обновлено') }
      else { await createDeliveryMethod(data); toast.success('Создано') }
      setSheetOpen(false); load(search, statusFilter)
    } catch (e: any) { toast.error(e.message || 'Ошибка') } finally { setSaving(false) }
  }

  const handleActivate = async (item: DeliveryMethod) => {
    try { await activateDeliveryMethod(item.id); toast.success('Активировано'); load(search, statusFilter) }
    catch (e: any) { toast.error(e.message) }
  }

  const handleDeactivate = async (item: DeliveryMethod) => {
    try { await deactivateDeliveryMethod(item.id); toast.success('Деактивировано'); load(search, statusFilter) }
    catch (e: any) { toast.error(e.message) }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <DictionarySheet open={sheetOpen} onClose={() => setSheetOpen(false)} title={editingItem ? 'Редактировать способ приёмки' : 'Добавить способ приёмки'} onSave={() => formRef.current?.requestSubmit()} isSaving={saving}>
        <DeliveryMethodForm defaultValues={editingItem} onSubmit={handleSave} formRef={formRef} />
      </DictionarySheet>
    </div>
  )
}
