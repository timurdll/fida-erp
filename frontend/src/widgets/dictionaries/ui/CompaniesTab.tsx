...
export function CompaniesTab() {
  ...
  const [editingItem, setEditingItem] = useState<Company | undefined>()
  ...
  const load = useCallback(async (s: string, sf: typeof statusFilter) => {
    ...
    try {
      ...
      setItems(await getCompanies({ isActive, search: s || undefined }))
    } catch { toast.error('Ошибка загрузки компаний') } finally { setLoading(false) }
  }, [])
  ...
  const handleSave = async (data: CreateCompanyDto) => {
    setSaving(true)
    try {
      if (editingItem) { await updateCompany(editingItem.id, data); toast.success('Компания обновлена') }
      else { await createCompany(data); toast.success('Компания создана') }
      setSheetOpen(false)
      load(search, statusFilter)
    } catch (e: any) { toast.error(e.message || 'Ошибка сохранения') } finally { setSaving(false) }
  }

  const handleActivate = async (item: Company) => {
    try { await activateCompany(item.id); toast.success('Компания активирована'); load(search, statusFilter) }
    catch (e: any) { toast.error(e.message) }
  }

  const handleDeactivate = async (item: Company) => {
    try { await deactivateCompany(item.id); toast.success('Компания деактивирована'); load(search, statusFilter) }
  ...
  return (
    <div className="space-y-4">
        ...
        {loading ? (
          ...
          <table className="w-full">
            <thead><tr className="border-b border-border">
              ...
              <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Функция</th>
              <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">БИН</th>
              <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Тип</th>
              <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Телефон</th>
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                <tr key={item.id} className={`border-b border-border transition-colors hover:bg-background-elevated cursor-pointer ${i % 2 === 1 ? 'bg-foreground/[0.02]' : ''}`}
                  ...
                  <td className="px-4 py-3 text-sm text-muted-foreground">{item.bin || '—'}</td>
                  <td className="px-4 py-3 text-sm text-muted-foreground">{CompanyTypeLabel[item.type]}</td>
                  <td className="px-4 py-3 text-sm text-muted-foreground">{item.contactPhone || '—'}</td>
                  <td className="px-4 py-3"><StatusBadge isActive={item.isActive} /></td>
                  <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon-sm" className="text-muted-foreground hover:text-foreground"><MoreHorizontal className="h-4 w-4" /></Button>
                      </DropdownMenuTrigger>
      ...
      <DictionarySheet
        ...
        title={editingItem ? 'Редактировать компанию' : 'Добавить компанию'}
        onSave={() => formRef.current?.requestSubmit()}
        isSaving={saving}
      >
        <CompanyForm defaultValues={editingItem} onSubmit={handleSave} formRef={formRef} />
      </DictionarySheet>
    </div>
  )
}
