...
import { ConstructionForm } from '@/features/dictionaries/ui/ConstructionForm'
import { getConstructions, createConstruction, updateConstruction, activateConstruction, deactivateConstruction } from '@/entities/construction/api/constructionApi'
...
export function ConstructionsTab() {
  ...
  const load = useCallback(async (s: string, sf: typeof statusFilter) => {
    setLoading(true)
    try { setItems(await getConstructions({ isActive: sf === 'all' ? undefined : sf === 'active', search: s || undefined })) }
  ...
  const handleSave = async (data: CreateConstructionDto) => {
    setSaving(true)
    try {
      if (editingItem) { await updateConstruction(editingItem.id, data); toast.success('Обновлено') }
      else { await createConstruction(data); toast.success('Создано') }
      setSheetOpen(false); load(search, statusFilter)
    } catch (e: any) { toast.error(e.message || 'Ошибка') } finally { setSaving(false) }
  }

  const handleActivate = async (item: Construction) => {
    try { await activateConstruction(item.id); toast.success('Активировано'); load(search, statusFilter) }
    catch (e: any) { toast.error(e.message) }
  }

  const handleDeactivate = async (item: Construction) => {
    try { await deactivateConstruction(item.id); toast.success('Деактивировано'); load(search, statusFilter) }
    catch (e: any) { toast.error(e.message) }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <DictionarySheet open={sheetOpen} onClose={() => setSheetOpen(false)} title={editingItem ? 'Редактировать конструкцию' : 'Добавить конструкцию'} onSave={() => formRef.current?.requestSubmit()} isSaving={saving}>
        <ConstructionForm defaultValues={editingItem} onSubmit={handleSave} formRef={formRef} />
      </DictionarySheet>
    </div>
  )
}
