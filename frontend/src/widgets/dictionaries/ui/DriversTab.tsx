...
export function DriversTab() {
  ...
  const [editingItem, setEditingItem] = useState<Driver | undefined>()
  const [saving, setSaving] = useState(false)
  const formRef = useRef<HTMLFormElement>(null)

  const load = useCallback(async (s: string, sf: typeof statusFilter) => {
    setLoading(true)
    try { setItems(await getDrivers({ isActive: sf === 'all' ? undefined : sf === 'active', search: s || undefined })) }
  ...
  const handleSave = async (data: CreateDriverDto) => {
    setSaving(true)
    try {
      if (editingItem) { await updateDriver(editingItem.id, data); toast.success('Водитель обновлён') }
      else { await createDriver(data); toast.success('Водитель создан') }
  ...
  return (
    <div className="space-y-4">
      ...
      <DictionarySheet open={sheetOpen} onClose={() => setSheetOpen(false)} title={editingItem ? 'Редактировать водителя' : 'Добавить водителя'} onSave={() => formRef.current?.requestSubmit()} isSaving={saving}>
        <DriverForm defaultValues={editingItem} onSubmit={handleSave} formRef={formRef} />
      </DictionarySheet>
    </div>
  )
}
