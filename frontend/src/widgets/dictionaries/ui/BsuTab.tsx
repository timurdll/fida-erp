...
export function BsuTab() {
  const [items, setItems] = useState<Bsu[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState<'all' | 'active' | 'inactive'>('active')
  const [sheetOpen, setSheetOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<Bsu | undefined>()
  ...
  const load = useCallback(async (s: string, sf: typeof statusFilter) => {
    setLoading(true)
    try {
      setItems(await getBsuList({ isActive: sf === 'all' ? undefined : sf === 'active', search: s || undefined }))
  ...
  const handleSave = async (data: CreateBsuDto) => {
    setSaving(true)
    try {
      if (editingItem) { await updateBsu(editingItem.id, data); toast.success('Обновлено') }
      else { await createBsu(data); toast.success('Создано') }
      setSheetOpen(false); load(search, statusFilter)
    } catch (e: any) { toast.error(e.message || 'Ошибка') } finally { setSaving(false) }
  }

  const handleDeactivate = async (item: Bsu) => {
    try { await deactivateBsu(item.id); toast.success('Деактивировано'); load(search, statusFilter) }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-lg border border-border bg-card">
        {loading ? (
          ...
          <table className="w-full">
            ...
            <tbody>
              {items.length === 0
                ? <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-muted-foreground">Ничего не найдено</td></tr>
      ...
      <DictionarySheet
        ...
        title={editingItem ? 'Редактировать БСУ' : 'Добавить БСУ'}
        onSave={() => formRef.current?.requestSubmit()}
        isSaving={saving}
      >
        <BsuForm defaultValues={editingItem} onSubmit={handleSave} formRef={formRef} />
      </DictionarySheet>
    </div>
  )
}
