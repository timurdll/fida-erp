...
export function TransportsTab() {
  ...
  const [editingItem, setEditingItem] = useState<Transport | undefined>()
  const [saving, setSaving] = useState(false)
  const formRef = useRef<HTMLFormElement>(null)

  const load = useCallback(async (s: string, sf: typeof statusFilter) => {
    setLoading(true)
    try { setItems(await getTransports({ isActive: sf === 'all' ? undefined : sf === 'active', search: s || undefined })) }
  ...
  const handleSave = async (data: CreateTransportDto) => {
    setSaving(true)
    try {
      if (editingItem) { await updateTransport(editingItem.id, data); toast.success('Транспорт обновлён') }
      else { await createTransport(data); toast.success('Транспорт создан') }
      setSheetOpen(false); load(search, statusFilter)
    } catch (e: any) { toast.error(e.message || 'Ошибка') } finally { setSaving(false) }
  }

  const handleActivate = async (item: Transport) => {
    try { await activateTransport(item.id); toast.success('Активировано'); load(search, statusFilter) }
    catch (e: any) { toast.error(e.message) }
  }

  const handleDeactivate = async (item: Transport) => {
    try { await deactivateTransport(item.id); toast.success('Деактивировано'); load(search, statusFilter) }
    catch (e: any) { toast.error(e.message) }
  }
  ...
  return (
    <div className="space-y-4">
        ...
        {loading ? <div className="p-4 space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-10 w-full" />)}</div> : (
          <table className="w-full">
            <thead><tr className="border-b border-border">
              ...
              <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Перевозчик</th>
              <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Тара (кг)</th>
              <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Допуск (%)</th>
              ...
              {items.length === 0 ? <tr><td colSpan={7} className="px-4 py-10 text-center text-sm text-muted-foreground">Ничего не найдено</td></tr>
                : items.map((item, i) => (
                  <tr key={item.id} className={`border-b border-border transition-colors hover:bg-background-elevated cursor-pointer ${i % 2 === 1 ? 'bg-white/[0.02]' : ''}`} onClick={() => { setEditingItem(item); setSheetOpen(true) }}>
                    ...
                    <td className="px-4 py-3 text-sm text-muted-foreground">{item.tare ?? '—'}</td>
                    <td className="px-4 py-3 text-sm text-muted-foreground">{item.tolerance}</td>
      ...
      <DictionarySheet open={sheetOpen} onClose={() => setSheetOpen(false)} title={editingItem ? 'Редактировать транспорт' : 'Добавить транспорт'} onSave={() => formRef.current?.requestSubmit()} isSaving={saving}>
        <TransportForm defaultValues={editingItem} onSubmit={handleSave} formRef={formRef} />
      </DictionarySheet>
    </div>
  )
}
